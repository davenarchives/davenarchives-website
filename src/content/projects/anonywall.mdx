...
- Quality checks: run `npm run lint`; smoke-test Google + anonymous auth, admin link visibility, Freedom Wall submit/report, Forum create/like/reply/report, Profile update/backfill, Admin report resolution.
- Deployment: `npm run build` outputs `dist/`; `vercel.json` rewrites `/*` -> `index.html` for SPA hosting (Vercel-ready). Set all `VITE_*` env vars (including `VITE_ADMIN_EMAILS`) in hosting.
- Troubleshooting: admin link missing (check `VITE_ADMIN_EMAILS`), empty Freedom Wall on first load (ensure `MessageContext` seeding + Firestore reads), auth issues (verify Firebase providers/env), permission errors (check rules for `messages`, `posts`, `reports`, `users`, `moderation_logs`).
